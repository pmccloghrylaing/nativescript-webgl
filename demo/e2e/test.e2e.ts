...
describe("WebGL tests", async function () {
    ...
    it("should log in via original button", async function () {
        ...
        const webglButton = await driver.findElementByAccessibilityId(FACEBOOK_BUTTON);
    ...
    it("should log out via custom button", async function () {
        const webglLogoutButton = await driver.findElementByAccessibilityId(CUSTOM_LOGOUT_BUTTON);
        await webglLogoutButton.click();
        const webglButton = await driver.findElementByAccessibilityId(FACEBOOK_BUTTON);
        expect(webglButton).to.exist;
    });
});